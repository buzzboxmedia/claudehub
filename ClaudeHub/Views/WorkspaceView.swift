...
struct SessionSidebar: View {
    ...
    @State private var sheetTasks: [GoogleSheetsService.SheetTask] = []
    @State private var isLoadingSheetTasks = false
    @State private var selectedSheetTask: GoogleSheetsService.SheetTask?
    ...
    func fetchSheetTasks() {
        isLoadingSheetTasks = true
        Task {
            do {
                let tasks = try await GoogleSheetsService.shared.fetchTasks(workspace: project.name)
                await MainActor.run {
                    sheetTasks = tasks
                    isLoadingSheetTasks = false
                }
            } catch {
                print("Failed to fetch sheet tasks: \(error)")
                await MainActor.run {
                    isLoadingSheetTasks = false
                }
            }
    ...
    /// Active tasks from Google Sheets
    var activeSheetTasks: [GoogleSheetsService.SheetTask] {
        sheetTasks.filter { $0.isActive }
    }

    /// Completed tasks from Google Sheets
    var completedSheetTasks: [GoogleSheetsService.SheetTask] {
        sheetTasks.filter { !$0.isActive }
    }
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        // Sheet Tasks Section (from Google Sheets)
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                Text("FROM SHEETS")
                                    .font(.system(size: 11, weight: .semibold))
                                ...
                                .buttonStyle(.plain)
                                .help("Refresh tasks from Google Sheets")
                            }
                            .padding(.horizontal, 16)
                            .padding(.top, 8)

                            if activeSheetTasks.isEmpty && !isLoadingSheetTasks {
                                Text("No tasks in spreadsheet")
                                    .font(.system(size: 11))
                                    .foregroundStyle(.tertiary)
                                    .padding(.horizontal, 16)
                                    .padding(.vertical, 8)
                                ...
                                LazyVStack(spacing: 4) {
                                    ForEach(activeSheetTasks) { task in
                                        SheetTaskRow(
                                            task: task,
                                            isSelected: selectedSheetTask?.id == task.id,
                                            onSelect: { selectedSheetTask = task },
                                            onComplete: { markSheetTaskComplete(task) }
                                        )
                                    }
                                }
                            }
        ...
        .sheet(item: $selectedSheetTask) { task in
            SheetTaskDetailView(task: task, workspace: project.name) {
                fetchSheetTasks()  // Refresh after changes
            }
        }
    }

    func markSheetTaskComplete(_ task: GoogleSheetsService.SheetTask) {
        Task {
            _ = try? await GoogleSheetsService.shared.updateTask(
                workspace: project.name,
                taskName: task.task,
                status: "done"
            )
...
struct SheetTaskRow: View {
    let task: GoogleSheetsService.SheetTask
    let isSelected: Bool
    let onSelect: () -> Void
    let onComplete: () -> Void
    @State private var isHovered = false
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button {
                ...
                Image(systemName: task.isActive ? "circle" : "checkmark.circle.fill")
                    .font(.system(size: 14))
                    .foregroundStyle(task.isActive ? Color.secondary : Color.green)
            }
            .buttonStyle(.plain)
...
// MARK: - Sheet Task Detail View

struct SheetTaskDetailView: View {
    let task: GoogleSheetsService.SheetTask
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if !task.project.isEmpty && task.project != "—" {
                            Text(task.project)
                                .font(.system(size: 11))
                                .foregroundStyle(.secondary)
                        }
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(task.description.isEmpty ? "No description" : task.description)
                            .font(.system(size: 13))
                            .foregroundStyle(task.description.isEmpty ? .tertiary : .primary)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack {
                            ...
                            .buttonStyle(.plain)
                            .disabled(progressNote.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || isSaving)
                        }
                    }
                }
                .padding()
